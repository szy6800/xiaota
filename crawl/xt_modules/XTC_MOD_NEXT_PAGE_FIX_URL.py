...
class XTC_MOD_NEXT_PAGE_FIX_URL(BaseMOD):
    def init(self, task):
        self.task = task
    ...
    def run(self):
        ...
        if self.task.env['run_times'] == 1:
            ...
            self.task.logger.debug('%s XTC_MOD_NEXT_PAGE_FIX_URL set start_page=%d for %d - %d + %d'%(self.task.site_info, self.start_page, self.total_page , succ_pages , self.re_crawl_page_num))
        else:
            #更新连续抓取成功页数
        ...
        self.task.logger.debug('%s XTC_MOD_NEXT_PAGE_FIX_URL info=%s'%(self.task.site_info, json.dumps(self.task.env['page_result']['info'], indent=2, ensure_ascii=False)))
        ...
        if now_page > 0 and now_page <= self.total_page and len(self.task.env['page_result']['reqs']) > 0:
            ...
            url+= str(eval(self.para['expr'], None, {'page':now_page, 'para':self.para, 'page_result':self.task.env['page_result']}))
        ...
        self.task.logger.debug('%s XTC_MOD_NEXT_PAGE_FIX_URL (%s/%s) next_reqs=%s'%(self.task.site_info, now_page,self.total_page,json.dumps(self.task.env['page_result']['next_reqs'], indent=2, ensure_ascii=False)))
        return {}
